...
class TestAsyncSession:
    ...
    async def test_async_post_request_form_data(self, session):
        ...
        form_data = {"async_session_form": "async_session_value", "async_form_test": "data"}
    ...
    async def test_async_post_request_with_files(self, session):
        ...
        try:
            with open(tmp_file_path, 'rb') as f:
                ...
                assert response.status_code == 200
                data = response.json()
                assert data["form"]["async_session_description"] == "Async session file upload"
    ...
    def test_async_session_header_management(self, session):
        ...
        assert session.session_headers["X-Async-Session-Custom"] == "async-custom-session-value"
...
class TestSessionIntegration:
    ...
    def test_sync_and_async_session_coexistence(self):
        ...
        sync_session = uf.Session(
            base_url="https://httpbin.org",
            headers={"X-Sync-Session": "sync-value"}
        )
        
        async_session = uf.AsyncSession(
            base_url="https://httpbin.org",
            headers={"X-Async-Session": "async-value"}
        )
