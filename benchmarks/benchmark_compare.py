...
import httpx
import aiohttp
from ultrafast_client import HttpClient, AsyncHttpClient
...
def print_comparison_table(results: List[BenchmarkResult]):
    ...
    print(f"{'='*80}")
    print(f"{'Library':20} | {'Mean Time':>10} | {'Req/Sec':>7} | {'Std Dev':>7} | {'Range':>15}")
    print(f"{'-'*80}")
    ...
    for result in sorted_results:
        ...
        print(f"{result.name:20} | {result.mean_time:8.2f}s | {result.req_per_sec:7.1f} | ±{result.std_dev:5.2f}s | {result.min_time:.2f}-{result.max_time:.2f}s | {speedup}")
...
def print_summary(results: List[BenchmarkResult]):
    ...
    valid_results = [r for r in results if r.req_per_sec != float('inf') and r.req_per_sec > 0]
    ...
    sync_results = [r for r in valid_results if not any(keyword in r.name.lower() for keyword in ['async', 'aiohttp'])]
    async_results = [r for r in valid_results if any(keyword in r.name.lower() for keyword in ['async', 'aiohttp'])]
    ...
    if ultrafast_results:
        ...
        for result in ultrafast_results:
            requests_result = next((r for r in valid_results if 'requests' in r.name.lower()), None)
            if requests_result:
...
def create_detailed_report(results: List[BenchmarkResult]):
    ...
    if sync_results:
        best_sync = max(sync_results, key=lambda r: r.req_per_sec)
        sync_libs = ", ".join([r.name for r in sorted(sync_results, key=lambda r: r.req_per_sec, reverse=True)])
        print(f"{'Synchronous':20} | {sync_libs[:48]:50} | {best_sync.req_per_sec:>8.1f}")
    
    # Async category
    async_results = [r for r in valid_results if any(keyword in r.name.lower() for keyword in ['async', 'aiohttp'])]
    if async_results:
        best_async = max(async_results, key=lambda r: r.req_per_sec)
        async_libs = ", ".join([r.name for r in sorted(async_results, key=lambda r: r.req_per_sec, reverse=True)])
        print(f"{'Asynchronous':20} | {async_libs[:48]:50} | {best_async.req_per_sec:>8.1f}")
    ...
    if valid_results:
        ...
        consistent = min(valid_results, key=lambda r: r.std_dev / r.mean_time if r.mean_time > 0 else float('inf'))
        print(f"🎯 For consistency: Use {consistent.name} (lowest variance)")
        
        # UltraFast specific recommendations
        ultrafast_sync = next((r for r in valid_results if 'ultrafast' in r.name.lower() and 'sync' in r.name.lower()), None)
        ultrafast_async = next((r for r in valid_results if 'ultrafast' in r.name.lower() and 'async' in r.name.lower()), None)
        ...
        if ultrafast_sync and ultrafast_async:
            ...
            if async_advantage > 5:
                print(f"💡 Recommendation: Use UltraFast async for high-throughput applications")
            else:
                print(f"💡 Both UltraFast modes are viable depending on your use case")
...
def main():
    ...
    print(f"🚀 UltraFast HTTP Client Benchmark Suite")
