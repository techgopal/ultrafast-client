...
impl SSEClient {
    ...
    pub fn connect(&mut self, url: &str) -> PyResult<()> {
        ...
        // Store the receiver
        *event_receiver_arc.lock()
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("Failed to acquire lock: {}", e)))? = Some(rx);
...
impl SSEEvent {
    ...
    pub fn is_keepalive(&self) -> bool {
        self.data.as_ref().map(|d| d.trim().is_empty()).unwrap_or(true) && self.event_type.is_none()
    }
    ...
    fn __str__(&self) -> String {
        format!("SSE Event: {}", self.data.as_ref().unwrap_or(&"<empty>".to_string()))
    }
}
...
impl SSEEventIterator {
    ...
    fn __next__(&mut self) -> PyResult<Option<SSEEvent>> {
        ...
        loop {
            let chunk_result = {
                let receiver_guard = receiver_arc.lock()
        ...
        future_into_py(py, async move {
            ...
            *event_receiver_arc.lock()
            ...
            let response = request.send().await
                .map_err(|e| pyo3::exceptions::PyConnectionError::new_err(format!("SSE connection failed: {}", e)))?;
