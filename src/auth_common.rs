...
pub fn apply_authentication(mut request: RequestBuilder, auth_config: Option<&AuthConfig>) -> RequestBuilder {
        ...
        match auth.auth_type {
            ...
            AuthType::OAuth2 => {
                if let Some(token) = auth.get_credential("access_token") {
                    let token_type = auth.get_credential("token_type").unwrap_or_else(|| "Bearer".to_string());
                    ...
                    match custom_type.as_str() {
                        "jwt" | "JWT" => {
                            if let (Some(token), Some(header)) = (
                                auth.get_credential("token"),
                                auth.get_credential("header")
                            ) {
                                let prefix = auth.get_credential("prefix").unwrap_or_else(|| "Bearer".to_string());
...
mod tests {
    ...
    fn test_api_key_header() {
        ...
        assert_eq!(auth.get_credential("header_name"), Some("X-API-Key".to_string()));
    }
    ...
    fn test_api_key_query() {
        ...
        assert_eq!(auth.get_credential("key"), Some("secret-key".to_string()));
        assert_eq!(auth.get_credential("param_name"), Some("api_key".to_string()));
    }
}
