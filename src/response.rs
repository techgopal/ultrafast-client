...
impl Response {
    ...
    pub fn text(&self) -> PyResult<String> {
        String::from_utf8(self.content.clone())
            .map_err(|e| pyo3::exceptions::PyUnicodeDecodeError::new_err(format!("Failed to decode response as UTF-8: {}", e)))
    }
    ...
    fn __repr__(&self) -> String {
        format!("Response(status_code={}, url='{}', content_length={})", 
                self.status_code, self.url, self.content.len())
    }
    ...
    pub fn get_timing_info(&self, py: Python) -> PyObject {
        ...
        dict.set_item("request_time", self.request_time).unwrap_or(());
        dict.set_item("response_time", self.response_time).unwrap_or(());
...
impl Response {
    ...
    pub(crate) async fn from_reqwest_response_async(response: reqwest::Response) -> PyResult<Self> {
        ...
        let content = response.bytes().await
            .map_err(|e| pyo3::exceptions::PyIOError::new_err(format!("Failed to read response body: {}", e)))?
            .to_vec();
    ...
    pub fn from_reqwest(response: reqwest::Response, runtime: &tokio::runtime::Runtime) -> PyResult<Self> {
        let status_code = response.status().as_u16();
        let url = response.url().to_string();
