...
impl HeaderPool {
    ...
    pub fn get(&self) -> PooledHeaders {
        ...
        let mut headers = pool.pop().unwrap_or_else(|| {
            AHashMap::with_capacity(self.default_capacity)
        });
...
impl HeaderCache {
    pub fn new() -> Self {
        ...
        let common_headers = [
            ...
            "content-type", "cookie", "date", "etag", "host", "if-match",
            "if-modified-since", "if-none-match", "last-modified", "location",
            "pragma", "server", "set-cookie", "transfer-encoding", "user-agent",
        ...
        let common_values = [
            ...
            "br", "close", "keep-alive", "no-cache", "max-age=0", "GET", "POST",
    ...
    pub fn get_common_headers(&self) -> std::collections::HashMap<String, String> {
        ...
        headers.insert("User-Agent".to_string(), "UltraFast-Client/0.2.0".to_string());
