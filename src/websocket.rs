...
impl WebSocketMessage {
    ...
    pub fn text(&self) -> PyResult<String> {
        match &self.text_data {
            ...
            None => Err(pyo3::exceptions::PyValueError::new_err("Not a text message")),
        }
    }
    ...
    pub fn data(&self) -> PyResult<&[u8]> {
        match &self.binary_data {
            ...
            None => Err(pyo3::exceptions::PyValueError::new_err("Not a binary message")),
        }
    }
        ...
        Python::with_gil(|py| {
            match self.message_type.as_str() {
                "text" => match &self.text_data {
                    ...
                    None => Err(pyo3::exceptions::PyValueError::new_err("Text message without content")),
                },
    ...
    fn __repr__(&self) -> String {
        match self.message_type.as_str() {
            "text" => format!("WebSocketMessage::Text('{}')", self.text_data.as_ref().unwrap_or(&"".to_string())),
            "binary" => format!("WebSocketMessage::Binary({} bytes)", self.binary_data.as_ref().map_or(0, |d| d.len())),
            "ping" => format!("WebSocketMessage::Ping({} bytes)", self.binary_data.as_ref().map_or(0, |d| d.len())),
            "pong" => format!("WebSocketMessage::Pong({} bytes)", self.binary_data.as_ref().map_or(0, |d| d.len())),
...
impl WebSocketClient {
    ...
    pub fn new(auto_reconnect: bool, max_reconnect_attempts: u32, reconnect_delay: f64) -> PyResult<Self> {
            ...
            loop {
                match tokio_tungstenite::connect_async(&url_clone).await {
                    Ok((ws_stream, _response)) => {
                        ...
                        {
                            let mut receiver_guard = message_receiver_arc.lock()
                    ...
                    Err(e) => {
                        if auto_reconnect && attempts < max_attempts {
                            ...
                            return Err(pyo3::exceptions::PyConnectionError::new_err(
                                format!("Failed to connect to WebSocket after {} attempts: {}", attempts, e)
                            ));
                        }
                    }
                }
        ...
        future_into_py(py, async move {
            if let Some(tx) = sender.as_ref() {
                tx.send(msg).map_err(|e| {
                    pyo3::exceptions::PyConnectionError::new_err(format!("Failed to send message: {}", e))
                })?;
                Ok(())
            } else {
                Err(pyo3::exceptions::PyConnectionError::new_err("Not connected"))
            }
        })
    }
    ...
    pub fn ping<'py>(&self, py: Python<'py>, data: Option<Vec<u8>>) -> PyResult<&'py PyAny> {
        ...
        future_into_py(py, async move {
            if let Some(tx) = sender.as_ref() {
                tx.send(msg).map_err(|e| {
                    pyo3::exceptions::PyConnectionError::new_err(format!("Failed to send ping: {}", e))
                })?;
                Ok(())
            } else {
                Err(pyo3::exceptions::PyConnectionError::new_err("Not connected"))
            ...
            if let Some(ref mut rx) = guard.as_mut() {
                match rx.try_recv() {
                    ...
                    Err(mpsc::error::TryRecvError::Disconnected) => {
                        Err(pyo3::exceptions::PyConnectionError::new_err("Connection lost"))
                    }
                }
            } else {
                Err(pyo3::exceptions::PyConnectionError::new_err("Not connected"))
        ...
        future_into_py(py, async move {
            ...
            if let Some(mut rx) = rx_option {
                let result = match tokio::time::timeout(
                    ...
                    Ok(None) => Err(pyo3::exceptions::PyConnectionError::new_err("Connection lost")),
